    ...
    def checkProjectileTroopCollision(self, shape1, shape2):
        if isinstance(shape2.userData, gobjects.Projectile):
            ...
            if isinstance(shape1.userData, gobjects.Troop):
                ...
                if not (projectile.ParentTroop is not None and troop == projectile.ParentTroop):
...
class MyContactFilter(box2d.b2ContactFilter):
    ...
    def ShouldCollide(self, shape1, shape2):
        ...
        collides = (filter1.maskBits & filter2.categoryBits) != 0 and (
...
class Physics(object):
    ...
    def __init__(self, parent):
        ...
        self.worldAABB.upperBound = (
            100 + self.parent.absolute.size.x * self.scale_factor,
            100 + self.parent.absolute.size.y * self.scale_factor + 100,
        )
        self.gravity = (0, 0)
        self.doSleep = True
...
class GameView(ui.RootElement):
    ...
    def __init__(self):
        ...
        self.backdrop_alpha_texture = drawing.texture.Texture(
            os.path.join(globals.dirs.images, "starfield_alpha.png")
        )
        ...
        self.backdrop = drawing.Quad(
            globals.backdrop_buffer, tc=numpy.array([(0, 0), (0, tiles.y), (tiles.x, tiles.y), (tiles.x, 0)])
        )
        self.backdrop_alpha = drawing.Quad(
            globals.backdrop_alpha_buffer,
            tc=numpy.array(
                [
                    (tiles.x, tiles.y),
                    (tiles.x, 0),
        ...
        self.backdrop.SetVertices(Point(0, 0), self.absolute.size, drawing.constants.DrawLevels.grid)
        self.backdrop_alpha.SetVertices(
            Point(0, 0), self.absolute.size, drawing.constants.DrawLevels.grid + 1
        )
        ...
        pygame.mixer.music.load(globals.pyinst.path(os.path.join("resource", "music", "theme.ogg")))
    ...
    def Draw(self):
        ...
        drawing.ResetState()
        drawing.Translate(
            self.parallax.x - self.viewpos.pos.x / 2, self.parallax.y - self.viewpos.pos.y / 2, 0
        )
