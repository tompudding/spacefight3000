...
class MainMenu(ui.RootElement):
    def __init__(self):
        ...
        self.play_button = ui.TextBoxButton(
            self, "Play", Point(0.22, 0.15), size=16, callback=self.Play, line_width=4
        )
        self.exit_button = ui.TextBoxButton(
            globals.screen_root, "Exit", Point(0.65, (0.15)), size=16, callback=self.Quit, line_width=4
        )
        ...
        pygame.mixer.music.load(globals.pyinst.path(os.path.join("resource", "music", "title_music.ogg")))
        self.StartMusic()
