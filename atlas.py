...
#This file taken from the blog post at http://omnisaurusgames.com/2011/06/texture-atlas-generation-using-python/

def Atlas(filenames, outname, size):

    #open all of the images
    ims = [(f, Image.open(f)) for f in filenames]
    #ims = [(f, im.crop((0,25,im.size[0],im.size[1]))) for (f,im) in ims]
    ...
    for idx, outimage in enumerate(outimages):
        outimage.save(outname + "_" + str(idx) + ".png")

    #write out an xml file that says where everything ended up
    with open(outname+'.txt','wb') as f:
        for fb in fullboxes:
            f.write("{name}:{outfile}_{filenum}.png:{x}:{y}:{w}:{h}\n".format(name    = fb[6]  ,
                                                                              outfile = outname,
                                                                              filenum = fb[1]  ,
                                                                              x       = fb[2]  ,
                                                                              y       = size - fb[3]-fb[5],
